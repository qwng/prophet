...
#' compile_stan_model_ht <- function(model) {
...
setup_dataframe <- function(m, df, initialize_scales = FALSE) {
  ...
  if (m$growth == 'logistic' | m$growth == "ht_logistic" | m$growth == "ht_logistic_cov") {
    ...
    df$cap_scaled <- df$cap / m$y.scale
  }
  return(list("m" = m, "df" = df))
}

#' Prepare dataframe for fitting or predicting of hierarchical timeseries.
#'
#' Adds a time index and scales y. Creates auxillary columns 't', 't_ix',
#' 'y_scaled', and 'cap_scaled'. These columns are used during both fitting
#' and predicting.
...
setup_dataframe_ht <- function(m, df, initialize_scales = FALSE) {
  ...
  if (initialize_scales) {
    m$y.scale <- apply(df$y, 2, function(x) max(abs(x)))
    m$start <- min(df$ds)
    m$t.scale <- time_diff(max(df$ds), m$start, "secs")
  }
  ...
  if (m$growth == 'logistic' | m$growth == "ht_logistic" | m$growth == "ht_logistic_cov") {
    ...
    df$cap_scaled <- t(t(df$cap) / m$y.scale)
  }
  return(list("m" = m, "df" = df))
}
...
set_auto_seasonalities <- function(m) {
  ...
  daily.disable <- ((time_diff(last, first) < 2) || (min.dt >= 0.5))
  fourier.order <- parse_seasonality_args(
...
#' Initialize hierachical time series logistic growth.
#'
#' Provides a strong initialization for logistic growth by calculating the
#' growth and offset parameters that pass the function through the first and
#' last points in the time series.
...
#'  growth function.
#'
ht_logistic_growth_init <- function(df) {
  i0 <- which.min(df$ds)
  i1 <- which.max(df$ds)
  T <- df$t[i1] - df$t[i0]
  # Force valid values, in case y > cap.
  r0 <- sapply(df$cap_scaled[i0, ] / df$y_scaled[i0, ], function(x) max(x, 1.01))
  r1 <- sapply(df$cap_scaled[i1, ] / df$y_scaled[i1, ], function(x) max(x, 1.01))
...
fit_ht.prophet <- function(m, df, ...) {
  ...
  out <- setup_dataframe_ht(m, history, initialize_scales = TRUE)
  history <- out$df
  m <- out$m
  m$history <- history
  m <- set_auto_seasonalities(m)
...
#' Predict using the prophet model for hierarchical time series.
#'
#' @param object Prophet object.
#' @param df Dataframe with dates for predictions (column ds), and capacity
#'  (column cap) if logistic growth. If not provided, predictions are made on
...
predict_ht.prophet <- function(object, df = NULL, ...) {
  if (is.null(df)) {
    ...
    out <- setup_dataframe_ht(object, df)
    df <- out$df
  }
  
  df$trend <- predict_trend_ht(object, df)
  
  # df <- df %>%
  #   dplyr::bind_cols(predict_uncertainty_ht(object, df)) %>%
  
  df$seasonal_components <- predict_seasonal_components_ht(object, df)
  
  df$yhat <- df$trend 
  
  for (i in 1:ncol(df$trend)) {
   df$yhat[, i] <- df$trend[, i]  + df$seasonal_components[[i]]$seasonal
  }
...
#' @return Vector with trend on prediction dates.
#'
predict_trend_ht <- function(model, df) {
  t <- df$t
  ...
  if (model$growth == 'ht_linear' | model$growth == 'ht_linear_cov') {
    ...
    trend <- NULL
    for (i in 1:ncol(model$history$y_scaled)) {
  ...
  return(t(t(trend) * model$y.scale))
}
...
#' @return Dataframe with seasonal components.
#'
predict_seasonal_components_ht <- function(m, df) {
  seasonal.features <- make_all_seasonality_features(m, df)
  lower.p <- (1 - m$interval.width)/2
  upper.p <- (1 + m$interval.width)/2
  ...
  if (nrow(components) > 0) {
    ...
    for (i in 1:ncol(m$history$y)) {
      component.predictions[[i]] <- components %>%
        dplyr::group_by(component) %>% dplyr::do({
          comp <- (as.matrix(seasonal.features[, .$col])
                   %*% m$params$beta[.$col, i]) * m$y.scale[i]
          dplyr::data_frame(ix = 1:nrow(seasonal.features),
                            mean = rowMeans(comp, na.rm = TRUE),
                            lower = apply(comp, 1, stats::quantile, lower.p,
                                          na.rm = TRUE),
    ...
    names(component.predictions) <- colnames(df$y)
    
    for (i in 1:ncol(m$history$y)) {
      component.predictions[[i]]$seasonal <- rowSums(
        component.predictions[[i]][unique(components$component)])
      }
    } else {
    component.predictions <- data.frame(seasonal = rep(0, nrow(df)))
  }
...
#' Prophet posterior predictive samples for hierarchical time series.
#'
#' @param m Prophet object.
#' @param df Prediction dataframe.
#'
#' @return List with posterior predictive samples for each component.
#'
sample_posterior_predictive_ht <- function(m, df) {
  # Sample trend, seasonality, and yhat from the extrapolation model.
  n.iterations <- 1
  samp.per.iter <- max(1, ceiling(m$uncertainty.samples / n.iterations))
  nsamp <- n.iterations * samp.per.iter  # The actual number of samples
  ...
  seasonal.features <- make_all_seasonality_features(m, df)
  sim.values <- list("trend" = array(0, c(nrow(m$history$y),  ncol(m$history$y), nsamp)),
                     "seasonal" = array(0, c(nrow(m$history$y),  ncol(m$history$y), nsamp)),
                     "yhat" = array(0, c(nrow(m$history$y),  ncol(m$history$y), nsamp)))
  
  for (i in 1:n.iterations) {
    # For each set of parameters from MCMC (or just 1 set for MAP),
    for (j in 1:samp.per.iter) {
      # Do a simulation with this set of parameters,
      sim <- sample_model_ht(m, df, seasonal.features, i)
      # Store the results
      for (key in c("trend", "seasonal", "yhat")) {
        sim.values[[key]][,,(i - 1) * samp.per.iter + j] <- sim[[key]]
      }
...
#' Prophet uncertainty intervals for hierachical time series
#'
#' @param m Prophet object.
#' @param df Prediction dataframe.
#'
#' @return Dataframe with uncertainty intervals.
#'
predict_uncertainty_ht <- function(m, df) {
  sim.values <- sample_posterior_predictive_ht(m, df)
  # Add uncertainty estimates
  lower.p <- (1 - m$interval.width)/2
  upper.p <- (1 + m$interval.width)/2
  
  intervals <- list(
    yhat = apply(sim.values$yhat, 1:2, function(x) stats::quantile(x, c(lower.p, upper.p))),
    trend = apply(sim.values$trend, 1:2, function(x) stats::quantile(x, c(lower.p, upper.p))),
    seasonal = apply(sim.values$seasonal, 1:2, function(x) stats::quantile(x, c(lower.p, upper.p)))
  )
...
#' Simulate observations from the extrapolated generative model for 
...
#' @return List of trend, seasonality, and yhat, each a vector like df$t.
#'
sample_model_ht <- function(m, df, seasonal.features, iteration) {
  trend <- sample_predictive_trend_ht(m, df)
  
  beta <- m$params$beta
  seasonal <- t(t((as.matrix(seasonal.features) %*% beta)) * m$y.scale)
  
  sigma <- m$params$sigma_obs
  noise <- t(t(matrix(stats::rnorm(nrow(m$history$y) * ncol(m$history$y), mean = 0, sd = sigma),
                  nrow = nrow(m$history$y), 
                  ncol = ncol(m$history$y))) * m$y.scale)
...
#' Simulate the trend using the extrapolated generative model for 
#' hierarchical time series.
#'
#' @param model Prophet object.
#' @param df Prediction dataframe.
...
#' @return Vector of simulated trend over df$t.
#'
sample_predictive_trend_ht <- function(model, df) {
  k <- model$params$k
  param.m <- model$params$m
  deltas <- model$params$delta
  ...
  # Get the empirical scale of the deltas, plus epsilon to avoid NaNs.
  lambda <- colMeans(abs((deltas))) + 1e-8
  # Sample deltas
  deltas.new <- sapply(lambda, function(x) 
    extraDistr::rlaplace(n.changes, mu = 0, sigma = x))
  
  # Combine with changepoints from the history
  changepoint.ts <- c(model$changepoints.t, changepoint.ts.new)
  deltas <- rbind(deltas, deltas.new)
  trend <- NULL
...
#'
#' @export
make_future_dataframe_ht <- function(m, periods, freq = 'day',
                                  include_history = TRUE) {
  # For backwards compatability with previous zoo date type,
  if (freq == 'm') {
    freq <- 'month'
